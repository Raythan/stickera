#!/usr/bin/env python3
"""Baixa imagens por termo (DuckDuckGo Images) no diretório atual.

Uso:
  python imgdl.py "banana;maçã|pirulito" "3|4|5"
  python imgdl.py "gato|cachorro" "5"              # 5 imagens para CADA termo
  python imgdl.py "Abacate;Banana" "5" 1.5 10     # delay download 1.5s, busca 10s

Termos: separados por ; ou |
Quantidades: um único número → vale para todos; vários → um por termo (; ou |).
Arquivos já existentes (ex.: Abacate_03.jpg) são pulados — pode rodar de novo após rate limit.

Dependência (recomendado):
...
def parse_terms(terms_arg: str) -> list[str]:
    terms = split_list(terms_arg)
    if not terms:
        raise SystemExit("Nenhum termo na lista (use ; ou | entre palavras).")
    return terms
...
def main() -> None:
    ...
    counts = parse_counts(sys.argv[2], len(terms))

    download_delay = DEFAULT_DOWNLOAD_DELAY_SEC
    if len(sys.argv) > 3:
        try:
            download_delay = max(0.0, float(sys.argv[3]))
        except ValueError:
            raise SystemExit(f"Delay download inválido: {sys.argv[3]!r}") from None

    search_delay = default_search_delay(len(terms), download_delay)
    if len(sys.argv) > 4:
        try:
            search_delay = max(0.0, float(sys.argv[4]))
        except ValueError:
            raise SystemExit(f"Delay busca inválido: {sys.argv[4]!r}") from None
